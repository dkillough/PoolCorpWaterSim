  ...
  constructor(eye, center, up) {
    ...
    window.addEventListener("mousemove", (mouse) => {
      ...
      if (side == 1) {
        // get camera matrix
        var camVec = vec3.sum(
          Camera.right().scale(-dx),
          up().scale(dy)
        );
        var lookDir = this.camera.target().subtract(this.camera.pos());
        // get axis
        var axis = vec3.cross(camVec, lookDir);
        // rotate
        this.camera.rotate(
          axis,
          0.05,
          this.fps ? this.camera.pos() : this.camera.target()
        );
      } else if (side == 2) {
        // zoom in and out
        this.camera.offset(this.camera.forward(), 0.1 * Math.sign(dy));
      }
      // set the new prevs
      this.prevX = mouse.screenX;
      this.prevY = mouse.screenY;
