  ...
  render(deltaTime, totalTime, viewMatrix, projectionMatrix){
    ...
    for(let i = 0; i < this.waterColors.length; i++) {
      if(this.waterColors[i].checked) {
        console.log(this.waterColors[i].id);
        let theWaterColor = this.calculateWaterColor(this.waterColors[i].id);
        console.log(theWaterColor);
        this.GL.uniform4fv(uColor, theWaterColor);
        checked = true;
        break;
      }
    }
